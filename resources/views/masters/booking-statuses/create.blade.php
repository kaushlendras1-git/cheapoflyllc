    ...
    <div class="lob-header d-flex align-items-center justify-content-between">
        <div>
            <h2 class="lob-title mb-1">
                <span class="iconify" data-icon="mdi:book-plus-outline"
        ...
        <nav aria-label="breadcrumb" class="lob__breadcrumb">
            <ol class="lob__breadcrumb-list mb-0">
                ...
                <li class="lob__breadcrumb-item active" aria-current="page">
                    <span class="iconify lob__breadcrumb-icon" data-icon="mdi:book-plus-outline"></span>
                    Add Booking Status
                </li>
            </ol>
        </nav>
                ...
                <form method="POST" action="{{ route('booking-status.store') }}"
                    ...
                    <div class="row g-4 align-items-end">
                        ...
                        <div class="col-md-12 mt-3">
                            ...
                            <div class="d-flex flex-wrap gap-3">
                                @foreach(['Sales', 'CCV', 'Billing', 'Changes', 'Quality', 'Charge Back'] as $dept)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="{{ strtolower($dept) }}"
                                        name="departments[]" value="{{ $dept }}">
                                    <label class="form-check-label" for="{{ strtolower($dept) }}">
                                        {{ $dept }}
                                    </label>
                                </div>
                                @endforeach
