    ...
    <div class="lob-header d-flex align-items-center justify-content-between">
        <div>
            <h2 class="lob-title mb-1">
                <span class="iconify" data-icon="mdi:domain" style="vertical-align: middle; font-size: 14px"></span>
                Company Reports
            </h2>
        </div>
        ...
        <div class="col-12">
            ...
            <div class="lob-card p-4">
                <form method="GET" action="{{ route('reports.company') }}"
                    ...
                    <!-- Export Button -->
                    <div class="col-md-1 d-flex align-items-end">
                        <a href="{{ route('reports.company.export', request()->query()) }}"
                            class="btn btn-primary w-100 d-flex align-items-center justify-content-center gap-2 button-style px-4 py-3"
            ...
            <div class="lob-card p-4 mt-4">
                <div class="table-container table-2">
                        ...
                        <table class="table align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="text-success">${{ number_format($lob->net_profit ?? 0, 2) }}</span>
                                    </td>
                                    <td><span class="text-info">${{ number_format($lob->call_cost ?? 0, 2) }}</span>
                                    </td>
                                    <td>${{ number_format($lob->gross_amount ?? 0, 2) }}</td>
                                    <td>${{ number_format($lob->net_amount ?? 0, 2) }}</td>
                                    <td><span class="text-danger">${{ number_format($lob->charge_back ?? 0, 2) }}</span>
                                    </td>
                                    <td><span class="text-warning">${{ number_format($lob->refund ?? 0, 2) }}</span>
                                    </td>
                                    <td><span class="text-success">${{ number_format($lob->net_profit ?? 0, 2) }}</span>
                                    </td>
                                    <td><span
                                            class="badge bg-label-primary">{{ number_format($lob->avg_qc_score ?? 0, 1) }}</span>
                                    </td>
                    ...
                    <div class="pagination-container">
                        {{ $lobReports->appends(request()->query())->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
