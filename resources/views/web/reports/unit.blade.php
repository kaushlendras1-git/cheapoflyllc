...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="upper-titles d-flex align-items-center justify-content-between mb-4">
        <h2 class="mb-0">Unit Reports (Manager)</h2>
        <div class="breadcrumb">
            <a class="active" href="#">Dashboard</a>
            <a class="active" aria-current="page">Unit Reports</a>
        </div>
    </div>
        ...
        <div class="col-md-12">
            ...
            <div class="card p-4">
                <form method="GET" action="{{ route('reports.unit') }}">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <div class="add-follow-btn export-btn">
                            <a data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Export To Excel" href="{{ route('reports.unit.export', request()->query()) }}" class="btn btn-success px-4 py-3 gap-1 w-auto button-style">
                ...
                <div class="booking-table-wrapper py-2 crm-table">
                        ...
                        <tbody>
                            @forelse($agents as $index => $agent)
                                <tr>
                                    ...
                                    <td>{{ $agent->name }}</td>
                                    <td><span class="text-danger">${{ number_format($agent->charge_back ?? 0, 2) }}</span></td>
                                    <td><span class="text-warning">${{ number_format($agent->refund ?? 0, 2) }}</span></td>
                                    <td>${{ number_format($agent->gross_amount ?? 0, 2) }}</td>
                                    <td>${{ number_format($agent->net_amount ?? 0, 2) }}</td>
                                    <td><span class="text-success">${{ number_format($agent->net_profit ?? 0, 2) }}</span></td>
                                    <td><span class="badge bg-primary">{{ number_format($agent->avg_qc_score ?? 0, 1) }}</span></td>
                    ...
                    <div class="mt-3">
                        {{ $agents->appends(request()->query())->links('vendor.pagination.bootstrap-5') }}
                    </div>
                </div>
                
                <!-- Chart Section -->
                <div class="mt-5">
                    <h4>Unit Performance Chart</h4>
...
new Chart(ctx, {
    ...
    options: {
        ...
        plugins: {
            title: {
                display: true,
                text: 'Unit Performance Metrics Over Time'
            },
            legend: {
                display: true,
                position: 'top'
